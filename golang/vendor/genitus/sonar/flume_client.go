...
type FlumeClient struct {
	...
	// span buffer
	chMetricBuffer chan *flume.ThriftFlumeEvent
	...
	metricQueue [] *flume.ThriftFlumeEvent
	// socket
...
func (fc *FlumeClient) init(metricBuffer chan *flume.ThriftFlumeEvent, wg *sync.WaitGroup) error {
	fc.initSucc = false
...
func (fc *FlumeClient) sendLogs() {
	...
	rs, err := fc.rpcClient.AppendBatch(batch)
