...
var (
	...
	// runtime metric, global & consumer lvel; [outer-code]
	onceType int64 = 0
	onceIP = ""
	oncePort = ""
	onceServiceName = ""

	// flume client hbase send speed
	consumerHBaseSendGauge int64
	// flume client oss send speed
	consumerOSSSendGauge int64
...
func repoterHerder() {
	...
	for {
		...
		atomic.StoreInt64(&consumerHBaseSendGauge, 0)
		atomic.StoreInt64(&consumerOSSSendGauge, 0)
		...
		var metaStr = fmt.Sprintf(`"meta": {"schema": "%s", "topic": "%s", "version": "%s"}`,
			DEFAULT_SCHEMA_NAME_VALUE, MESSAGE_QUEUE_KAFKA_TOPIC_VALUE, QUIVER_VERSION)

		// 2. parameter
		var trace = fmt.Sprintf(`"trace": { "deliver_enable": %v, "dump_enable": %v, "spill_enable": %v, "flush_retry_count": %d, "consumer_num": %d }`,
			DeliverEnable, DumpEnable, SpillEnable, FlushRetryCount, consumerClientNum)
		...
		var svc = fmt.Sprintf(`"svc": { "type": "%d", "ip": "%s", "port": "%s", "svn": "%s"}`,
			onceType, onceIP, oncePort, onceServiceName)
		...
		for _, c := range consumerClients {
			channelCap += cap(c.eventBufferChannel)
			channelLen += len(c.eventBufferChannel)
		}
		var consumerStr = fmt.Sprintf(`"consumers": { "channel_cap": %d, "channel_len": %d, "send_gauge": %d, "hbase_send_gauge": %d, "oss_send_gauge": %d }`, channelCap, channelLen, atomic.LoadInt64(&consumerSendGauge), atomic.LoadInt64(&consumerHBaseSendGauge), atomic.LoadInt64(&consumerOSSSendGauge))
