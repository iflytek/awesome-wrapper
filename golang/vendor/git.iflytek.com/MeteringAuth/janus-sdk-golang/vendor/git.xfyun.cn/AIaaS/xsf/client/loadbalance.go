...
func (lb *loadBalance) findWithID(lbp *LBParams, id string) (tmp bool, conn *SFConn, e error) {
	...
	if len(addr) > 0 {
		...
		var i uint
		for i = 0; i < lb.r; i++ {
			lb.log.Infof("loadBalance.findWithID id: %s, addr:%s", id, addr[addrIx%len(addr)])
			var c *grpc.ClientConn
	...
	//经过重试后，仍未找到
	lb.log.Errorw("loadBalance.FindWithID can't find addr", "logId", lbp.logId, "name", lbp.name)
	if e != nil {
		return false, nil, e
	}
