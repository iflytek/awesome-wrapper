...
type slidingDelayWindow struct {
	//循环队列
...
func newSlidingDelayWindow(timePerSlice time.Duration, winSize int64) *slidingDelayWindow {
	slidingWindowInst := slidingDelayWindow{}
	slidingWindowInst.Init(timePerSlice, winSize)
	return &slidingWindowInst
}

//清理闲时的节点数据
func (s *slidingDelayWindow) reset() {
...
func (s *slidingDelayWindow) Init(timePerSlice time.Duration, winSize int64) {
...
func (s *slidingDelayWindow) locationIndex() int64 {
	return (time.Now().UnixNano() / int64(s.timePerSlice)) % s.timeSliceSize
}

//对时间片做增加操作，并返回窗口中所有的计数总和
func (s *slidingDelayWindow) setDur(dur int64) {
...
//入参为tps的s
func (s *slidingDelayWindow) getStats(unit time.Duration) (max, min, avg int64) {
	var index = s.locationIndex()
...
func (s *slidingDelayWindow) clearBetween(fromIndex, toIndex int64) {
