...
func deathSentence() {
	...
	*/
	...
	deathSentenceOnce1.Do(func() {
		{
			/*
				最高优先级
			*/
			fmt.Printf("deal with killerFirstPriority\n")
			...
			/*
				执行高优先级
			*/
			fmt.Printf("deal with killerHighPriority\n")
			for handle, task := range KillerCheckList {
	...
	/*
		执行普通优先级
	*/
	fmt.Printf("deal with killerNormalPriority\n")
	for handle, task := range KillerCheckList {
	...
	deathSentenceOnce2.Do(func() {
		{
			/*
				执行低优先级
			*/
			fmt.Printf("deal with killerLowPriority\n")
			for handle, task := range KillerCheckList {
	...
	/*
		最低优先级
	*/
	fmt.Printf("deal with killerLastPriority\n")
	for handle, task := range KillerCheckList {
		...
		case syscall.SIGTERM, syscall.SIGINT:
			{
				...
				if len(KillerCheckList) == 1 && func() bool {
					...
					fmt.Printf("KillerCheckList only contain finder,break.\n")
					signal.Stop(c)
					break end
				}
				KillerMu.RUnlock()
