...
func (f *finderAdapter) Register(addr string, version string) (bool, error) {
	...
	if finder, finderOk := f.finderMap[addr]; finderOk {
		fmt.Printf("about to call finder.RegisterSrvWithAddr -> addr:%v,version:%s\n", addr, version)
		...
		fmt.Printf("success call finder.RegisterSrvWithAddr -> addr:%v,version:%s\n", addr, version)
		return true, nil
	}
	return false, nil
}
func (f *finderAdapter) Closeout() {
	fmt.Println("about to UnRegisterSrv")
	f.finderRWMU.RLock()
	defer f.finderRWMU.RUnlock()
