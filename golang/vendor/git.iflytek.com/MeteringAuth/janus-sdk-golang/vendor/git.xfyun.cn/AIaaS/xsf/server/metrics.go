...
var (
	...
	slidingErrCodeWindows []*slidingErrCodeWindow
	slidingDelayWindows   []*slidingDelayWindow
...
func AddSlidingErrCodeWindows(item *slidingErrCodeWindow) {
	slidingRwMu.Lock()
	defer slidingRwMu.Unlock()
	slidingErrCodeWindows = append(slidingErrCodeWindows, item)
}
func AddSlidingDelayWindows(item *slidingDelayWindow) {
	slidingRwMu.Lock()
	defer slidingRwMu.Unlock()
...
func (r *Registry) init() error {
	if r.ready {
		fmt.Println("metrics repeat initialization")
