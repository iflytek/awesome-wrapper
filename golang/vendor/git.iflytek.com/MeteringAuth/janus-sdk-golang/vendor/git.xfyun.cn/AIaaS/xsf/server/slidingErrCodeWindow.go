...
type slidingErrCodeWindow struct {
	//循环队列
...
func newSlidingErrCodeWindow(timePerSlice time.Duration, winSize int64) *slidingErrCodeWindow {
	slidingWindowInst := slidingErrCodeWindow{}
	slidingWindowInst.Init(timePerSlice, winSize)
	return &slidingWindowInst
}

//清理闲时的节点数据
func (s *slidingErrCodeWindow) reset() {
...
func (s *slidingErrCodeWindow) Init(timePerSlice time.Duration, winSize int64) {
...
func (s *slidingErrCodeWindow) locationIndex() int64 {
	return (time.Now().UnixNano() / int64(s.timePerSlice)) % s.timeSliceSize
}

//对时间片做增加操作，并返回窗口中所有的计数总和
func (s *slidingErrCodeWindow) setErrCode(errCode int64) {
...
//入参为tps的s
func (s *slidingErrCodeWindow) getStats(unit time.Duration) map[int64]int64 {
	var index = s.locationIndex()
...
func (s *slidingErrCodeWindow) merge(m1 map[int64]int64, m2 map[int64]*int64) {
	for k, v := range m2 {
		m1[k] += *v
	}
}
...
func (s *slidingErrCodeWindow) clearBetween(fromIndex, toIndex int64) {
