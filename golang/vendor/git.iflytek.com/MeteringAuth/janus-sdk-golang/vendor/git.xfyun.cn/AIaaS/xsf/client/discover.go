...
// next 获取下一个地址
func (m *cMap) NextInList(s int) []string {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if m.cl == nil {
		return nil
	}
	return m.cl.nextInList(s)
}
...
type ServiceDiscoryCallBack = func(string, []*finder.ServiceInstance)
...
func (sd *serviceDiscovery) OnServiceInstanceConfigChanged(name string, version string, instance string, config *finder.ServiceInstanceConfig) bool {
	fmt.Println("OnServiceInstanceConfigChanged")
	//	s,ok := sd.ss[name]
	...
	if config.IsValid {
		fmt.Println("OnServiceInstanceConfigChanged insert adddr: ", instance)

		s.(*service).addrs.insert(instance)
	} else {
		fmt.Println("OnServiceInstanceConfigChanged delete adddr: ", instance)
...
func (sd *serviceDiscovery) OnServiceConfigChanged(name, version string, config *finder.ServiceConfig) bool {
	fmt.Println("OnServiceConfigChanged")
...
func (sd *serviceDiscovery) OnServiceInstanceChanged(name, version string, instances []*finder.ServiceInstanceChangedEvent) bool {
	fmt.Println("OnServiceInstanceChanged")
	...
	for _, v := range instances {
		if v.EventType == finder.INSTANCEADDED {
			for _, inst := range v.ServerList {
				...
				fmt.Println("OnServiceInstanceChanged insert addrs:", inst.Addr)
			...
			for _, inst := range v.ServerList {
				...
				fmt.Println("OnServiceInstanceChanged delete addrs:", inst.Addr)

				s.addrs.delete(inst.Addr)
			}
		}
		for _, cbf := range sd.callBackFunc {
			cbf(string(v.EventType), v.ServerList)
		}
	}
	return true
}
...
func (sd *serviceDiscovery) findAll(version, name, logId string, log *Logger) (*service, error) {
	...
	for _, cbf := range sd.callBackFunc {
		cbf(string(finder.INSTANCEADDED), srv[name+"_"+version].ProviderList)
	}
	log.Infow(
		"dealing hasVal",
