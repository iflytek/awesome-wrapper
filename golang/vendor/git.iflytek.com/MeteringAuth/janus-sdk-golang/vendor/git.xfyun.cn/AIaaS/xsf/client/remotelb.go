...
func (rlb *remoteLB) Find(param *LBParams) ([]string, error) {
	...
	if param.directEngIp != "" {
		return []string{param.directEngIp}, nil
	}
	...
	for i := 0; i < rlb.retry+1; i++ {
		...
		if e == nil && len(addr) > 0 {
			...
			return nil, INVALIDLB
		}
	}
	if grpcErr != nil {
		return nil, INVALIDRMLB
	}
	return nil, EINVALIDADDR
}
