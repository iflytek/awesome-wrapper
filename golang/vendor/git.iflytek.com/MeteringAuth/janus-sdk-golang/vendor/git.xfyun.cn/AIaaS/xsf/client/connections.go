...
func (cm *connPoolMeta) newconnPool(addr string, ms time.Duration) (*grpc.ClientConn, error) {
				...
				for {
					if cm.size < cm.max {
						func() {
							...
							defer cancel()
							connPool, e := grpc.DialContext(ctx, addr, grpc.WithInsecure(), grpc.WithAuthority(addr),
								grpc.WithBlock(), grpc.WithReadBufferSize(cm.rbuf), grpc.WithWriteBufferSize(cm.wbuf),
	...
	connPool, e := grpc.DialContext(ctx, addr, grpc.WithInsecure(), grpc.WithAuthority(addr), grpc.WithBlock(),
		grpc.WithReadBufferSize(cm.rbuf), grpc.WithWriteBufferSize(cm.wbuf), grpc.WithMaxMsgSize(cm.maxReceive))
	if e == nil {
		if !cm.insert(&connPoolNode{cc: connPool}) {
			connPool.Close()
...
func (c *connPool) get(addr string, id string) (*grpc.ClientConn, error) {
	...
	if !ok {
		...
		connPool, e = st.alloc(addr, time.Duration(c.o.timeout)*time.Millisecond)
		if e == nil {
			// todo:: 判断insert失败移除st

			o, s := c.insertAddr(&addr, &id, &st)
			if !o { // 插入失败。则关闭所有的连接
				st.closeAll()
				return s.alloc(addr, time.Duration(c.o.timeout)*time.Millisecond)
			}
