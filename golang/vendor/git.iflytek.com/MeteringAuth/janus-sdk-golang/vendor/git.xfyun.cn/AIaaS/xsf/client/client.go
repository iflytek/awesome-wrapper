...
func (c *Client) newLB() error {
	if c != nil && c.cfg != nil {
		...
		// 初始化
		c.lb = newLB(c.Log, uint(r), LBMode(lbm), WithConLifeCycle(lc), WithConTimeOut(tm),
			WithConMax(p), WithConFindManger(c.fm),
			WithLBRetry(lbrty), WithLBTimeOut(lbtm), WithReadBufSize(rbuf), WithWriteBufSize(wbuf),
