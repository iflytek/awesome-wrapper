...
func (s *SessionManager) SetSessionData(sessionTag interface{}, svcData interface{}, callback func(sessionTag interface{}, svcData interface{}, Exception ...CallBackException)) error {
	...
	{
		//debug
		s.logger.Debugf("op:%v,tag:%v,now:%v,deadTime:%v,timeout:%v", "SetSessionData", sessionTag, time.Now(), time.Now().Add(s.Timeout), s.Timeout)
	}
	node := &Node{Callback: callback, Data: svcData, sessionTag: sessionTag, Deadtime: time.Now().Add(s.Timeout)}
		...
		case <-timer.C:
			{
				...
				for {
					...
					if ts.After(sessionDataNode.Deadtime) {
						s.logger.Infof("dealtimeout ->timestamp:%v,sessionTag:%v", ts, sessionDataNode.sessionTag)
						s.l.Remove(element)
						s.lMu.Unlock()
						s.mMu.Lock()
						if _, ok := s.m[sessionDataNode.sessionTag]; !ok {
							s.logger.Errorf("can't take of tag:%v of sessionDataNode", sessionDataNode.sessionTag)
							s.mMu.Unlock()
