...
func (c *Caller) createCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		return c.retryCall(apiVersion, service, r, tm, span, withCallReq(r))
	}
...
func (c *Caller) oneShortCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	tmpFlag, prx, conn, e := c.getPrxy(apiVersion, service, spnxt, withCallLogId(logId))
	if e != nil {
	...
	callBase := time.Now()
	s, errcode, e = c.doCall(prx, r, tm)
	c.cli.Log.Infow("record doCall perf", "fn", "oneShortCall", "dur", time.Since(callBase).Nanoseconds())
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		return c.retryCall(apiVersion, service, r, tm, span, withCallLogId(logId), withCallRetry(true))
	}
...
func (c *Caller) getSrv(service string) (addr []string, addrErr error) {
	...
	addr, addrErr = c.cli.lb.lbi.Find(lbp)
	return
}
...
func (c *Caller) GetHashAddr(hashKey string, svc string) (string, error) {
	...
	addr, err := c.cli.lb.lbi.Find(lbp)
