...
func (p *partitionProducer) internalFlushCurrentBatch() {
	...
	if batchData.ReadableBytes() > uint32(p.cnx.GetMaxMessageSize()) {
		p.log.WithError(errMessageTooLarge).
			WithField("size", batchData.ReadableBytes()).
			Errorf("MaxMessageSize %d", int(p.cnx.GetMaxMessageSize()))
		p.metrics.PublishErrorsMsgTooLarge.Inc()
		for _, cb := range callbacks {
			if sr, ok := cb.(*sendRequest); ok {
				sr.callback(nil, sr.msg, errMessageTooLarge)
			}
		}
		return
	}
...
func (p *partitionProducer) internalFlushCurrentBatches() {
	...
	for i := range batchesData {
		...
		// if msg is too large
		// If batching is enabled we compress all the payloads together before sending the batch
		if batchesData[i].ReadableBytes() > uint32(p.cnx.GetMaxMessageSize()) {
			p.log.WithError(errMessageTooLarge).
				WithField("size", batchesData[i].ReadableBytes()).
				Errorf("MaxMessageSize %d", int(p.cnx.GetMaxMessageSize()))
			p.metrics.PublishErrorsMsgTooLarge.Inc()
