...
func (rlb *remoteLB) Find(param *LBParams) ([]string, []string, error) {
	...
	addr, allAddrs, e := rlb.lbi.Find(param)
	param.log.Infow("fn:remoteLb-Find", "raw addr", addr, "allAddrs", allAddrs, "logId", param.logId)
	addr = func() []string {
		if !param.sort {
			return addr
		}
		tmp := sort.StringSlice(allAddrs)
		sort.Sort(tmp)
		param.log.Infow("fn:remoteLb-Find", "tmp addr", tmp, "logId", param.logId)
		return tmp
	}()
	param.log.Infow("fn:remoteLb-Find", "raw addr", addr, "allAddrs", allAddrs, "sort addr", addr, "sort", param.sort, "logId", param.logId)
	var raddrs []string
	var re error
	var grpcErr error
