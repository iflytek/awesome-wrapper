...
func (t *ToolBox) Init(bc BootConfig) error {
	...
	lbv2Client, lbv2ClientErr := func() (hermes hermesAdapter, hermesErr error) {
		if cfg.GetSection(HERMES) != nil {
			...
			if !lbAble {
				hermes.able = false
				loggerStd.Println("disable hermes.")
				return
			} else {
				hermes.able = true
			}

			lbTargets, lbTargetsErr := cfg.GetString(HERMES, HERMESLBTARGETS)
			if lbTargetsErr != nil {
				loggerStd.Printf("get lbTargets failed,err:%v.", lbTargetsErr)
			} else {
				loggerStd.Printf("got lbTargets:%v.", lbTargets)
			}
			...
			if len(lbprj) == 0 {
				lbprj = bc.CfgData.Project
			}
			lbgro, _ := cfg.GetString(HERMES, HERMESLBGROUP)
			if len(lbgro) == 0 {
				lbgro = bc.CfgData.Group
			}
			...
			if InitErr := hermes.Init(
				...
				WithHermesAdapterLbTargets(lbTargets),
	...
	if fcAble != 0 {
		...
		if router == ROUTER2SESSIONMANAGER {
			...
			t.Cache, SessionManagerErr = NewSessionManager(
				...
				WithSessionManagerRollTime(rollTime*time.Millisecond),
...
func (c *callserver) Call(ctx context.Context, in *utils.ReqData) (*utils.ResData, error) {
	...
	baseSpan := utils.FromMeta(meta, c.tm.ip, int32(c.tm.port), c.tm.serviceName, utils.SrvSpan)
	if baseSpan == nil {
		baseSpan = utils.NewSpan(utils.SrvSpan)
	}
	//span = span.WithName("Call").Start()
	baseSpan = baseSpan.WithName(in.Op).Start()
	baseSpan = baseSpan.WithRpcCallType()
	baseSpan.WithTag("xsf_srv->arrived", time.Now().String())
	baseSpan.End().Flush()

	span := baseSpan.Next(utils.SrvSpan)
	...
	span.WithTag("record call perf", strconv.FormatInt(dur, 10))
