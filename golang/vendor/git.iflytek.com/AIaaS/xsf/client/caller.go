...
func (c *Caller) createCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		c.cli.Log.Infow("doCall failed,begin retryCall", "brokenAddr", conn.Addr, "fn", "createCall", "dur", time.Since(callBase).Nanoseconds())
...
func (c *Caller) oneShortCall(apiVersion, service string, r *Req, tm time.Duration) (s *Res, errcode int32, e error) {
	...
	getPrxyBase := time.Now()
	tmpFlag, prx, conn, e := c.getPrxy(apiVersion, service, spnxt, withCallLogId(logId), withCallReq(r))
	spnxt.WithTag("getPrxy elapsed", strconv.FormatInt(time.Since(getPrxyBase).Nanoseconds(), 10))
	if e != nil {
	...
	spnxt.WithTag("doCall elapsed", strconv.FormatInt(dur, 10))
	c.cli.Log.Infow("record doCall perf",
		"fn", "oneShort:Call", "dur", dur, "tm", tm.String(), "errcode", errcode, "e", e)
	updateLbBase := time.Now()
	c.cli.updateLb(service, conn.Addr, s, errcode, e, dur)
	spnxt.WithTag("updateLb elapsed", strconv.FormatInt(time.Since(updateLbBase).Nanoseconds(), 10))
	if e != nil {
	...
	// 进入重试,换机器
	if e != nil && c.retry > 0 {
		c.cli.Log.Infow("doCall failed,begin retryCall", "brokenAddr", conn.Addr, "fn", "oneShortCall", "dur", time.Since(callBase).Nanoseconds())
...
func (c *Caller) getPrxy(apiVersion, service string, sp *Span, opt ...callOpt) (bool, utils.XsfCallClient, *SFConn, error) {
	...
	if len(c.lbname) != 0 { //启用了远端LB模式
		...
		lbp.WithSort(func() bool {
			if len(c.lbExt["sort"]) != 0 {
				switch c.lbExt["sort"] {
				case "true":
					return true
				case "false":
					return false
				default:
					c.cli.Log.Errorw("the lbExt.sort is illegal", "sort", c.lbExt["sort"], "logId", callCfgInst.logId)
					return false
				}
			}
			return false
